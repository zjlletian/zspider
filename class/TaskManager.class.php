...
class TaskManager {
	...
	static function getTask($hash){
		...
		if($task!=null){
            mysqli_query(self::$mycon,"update onprocess set uniqid='{$uniqid}',status=1,times=times+1,proctime=(SELECT unix_timestamp(now())),acktime=(SELECT unix_timestamp(now())+300+times*10+level*10),spider='{$GLOBALS['SPIDERNAME']}' where id={$task['id']} and status=0 limit 1");
		...
		if($task!=null){
			...
			//标记为正在处理
            mysqli_query(self::$mycon,"insert into onprocess values(null,'{$uniqid}','{$url}',{$task['level']},{$task['time']},{$task['type']},(SELECT unix_timestamp(now())),(SELECT unix_timestamp(now())+300+{$task['level']}*10),1,1,'{$GLOBALS['SPIDERNAME']}')");
